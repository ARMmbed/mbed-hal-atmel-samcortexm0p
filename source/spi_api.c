...
void spi_init(spi_t *obj, PinName mosi, PinName miso, PinName sclk)
{
    ...
    PinName ssel = NC; /*Chip select is removed from upper layer implementations.*/
    uint16_t baud = 0;
    uint32_t ctrla = 0;
    uint32_t ctrlb = 0;
...
void spi_format(spi_t *obj, int bits, int mode, spi_bitorder_t order)
{
    ...
    /* Sanity check arguments */
    MBED_ASSERT(obj);
    MBED_ASSERT(bits > 3 && bits < 17); /*Maximum of 16 bit transfer*/

    /* Disable SPI */
    spi_disable(obj);
    pSPI_S(obj)->bits = bits;
    ...
    if (order == SPI_MSB) {
        ctrla |= SPI_DATA_ORDER_MSB;
    } else {
        ctrla |= SPI_DATA_ORDER_LSB;
    }
